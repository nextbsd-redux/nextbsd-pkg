#!/bin/sh
# build.sh — runs INSIDE the FreeBSD VM (vmactions). Repackages the four
# component `continuous` artifacts into one pkg(8) package each (named after the
# source repo), plus a NextBSD-world meta, into a FLAT repo (out/repo/).
#
# Coherent-snapshot versioning: all four share one snapshot version this run, so
# the dep graph lines up. (Independent content-versioning + skip-unchanged-upload
# is a later optimization.) UNSIGNED for now; signing is a pre-production step.
set -eux
...
version: "${VER}"
comment: "${_comment}"
desc: "${_comment} — NextBSD ${VER} snapshot."
...
# dep helper -> UCL block pinning a package to this snapshot version
dep() { printf 'deps: { %s: { origin: "nextbsd/%s", version: "%s" } }\n' "$1" "$1" "$VER"; }
dep2() { printf 'deps: { %s: { origin: "nextbsd/%s", version: "%s" }, %s: { origin: "nextbsd/%s", version: "%s" } }\n' "$1" "$1" "$VER" "$2" "$2" "$VER"; }

# --- 1. NextBSD-freebsd-compat (the FreeBSD base: libc/libs, PAM, commands) ---
...
# --- 3. NextBSD-kernel-extensions (all kexts; root:wheel + go-w for OSKext) ---
...
# Tarballs carry their build-runner uid; OSKext requires root:wheel + no group/other write.
...
# --- 4. NextBSD-userland (the Darwin Tier 0-2 runtime + daemons) ---
...
# --- 5. NextBSD-world (meta: installs the whole OS) ---
mkdir -p stage/world
cat > /tmp/+MANIFEST <<UCL
name: NextBSD-world
origin: nextbsd/NextBSD-world
version: "${VER}"
comment: "NextBSD world meta-package (base + kernel + kernel-extensions + userland)"
desc: "Installs the complete NextBSD OS snapshot ${VER}."
maintainer: "dev@nextbsd.org"
www: "https://nextbsd.org"
abi: "${ABI}"
arch: "${ABI}"
prefix: "/"
deps: {
  NextBSD-freebsd-compat:  { origin: "nextbsd/NextBSD-freebsd-compat",  version: "${VER}" }
  NextBSD-kernel:          { origin: "nextbsd/NextBSD-kernel",          version: "${VER}" }
  NextBSD-kernel-extensions: { origin: "nextbsd/NextBSD-kernel-extensions", version: "${VER}" }
  NextBSD-userland:        { origin: "nextbsd/NextBSD-userland",        version: "${VER}" }
}
UCL
: > /tmp/plist
...
echo "=== packages ==="; ls -lh out/repo/*.pkg
pkg repo out/repo
